...
function TeamRoom() {
  ...
  const load = useCallback(async () => {
    ...
    if (!tm) {
      setLoadError(
        "This team isn't visible to your account. Open the invite link with the signed-in email, or ask your admin for a fresh invite.",
      );
      setLoaded(true);
      return;
    }
    setTeam(tm as Team);
    const [{ data: tt }, { data: ats }, { data: pl }, { data: cats }, { data: st }, { data: bd }] =
      await Promise.all([
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "auction_state",
          filter: `tournament_id=eq.${team.tournament_id}`,
        },
        () => load(),
      )
      .on(
  ...
  const currentCategory =
    currentPlayer && (currentPlayer as any).category_id
      ? categories.find((c) => c.id === (currentPlayer as any).category_id)
      : null;
  ...
  const placeBid = async (amount: number) => {
    ...
    const { data, error } = await supabase.rpc("place_bid", {
      p_tournament: team.tournament_id,
      p_player: currentPlayer.id,
      p_team: team.id,
      p_amount: amount,
    });
    setBidding(false);
  ...
  const sportSlug =
    SPORTS.find((s) => tournament.name.toLowerCase().includes(s.slug))?.slug || "cricket";
  ...
  return (
    <TeamRoomInner
      ...
      soldTeam={soldTeam}
      soldPrice={state?.last_sold_price ?? null}
      sport={sport}
    />
  );
}
...
interface InnerProps {
  ...
  soldPlayer: Player | null | undefined;
  soldTeam: Team | null | undefined;
...
function TeamRoomInner({
  ...
  const downloadSquadSummary = () => {
    ...
    squad.forEach((p) => {
      rows.push([`"${p.name}"`, `"${p.role || ""}"`, String(p.sold_price || 0)]);
    });
  ...
  return (
    ...
    >
      ...
      {tournament.banner_url && (
        <img src={tournament.banner_url} alt="" className="w-full h-32 md:h-48 object-cover" />
      )}
          ...
          {isLive && currentPlayer ? (
            <div className="space-y-6 animate-slide-up">
              <div className="rounded-xl gradient-neon p-6 text-primary-foreground flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs uppercase tracking-widest opacity-80">
                    On the block • ⏱ {timeLeft}s
                  </div>
                  <div className="text-3xl md:text-4xl font-bold mt-1 truncate">
                    {currentPlayer.name}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Stat
                  label="Current bid"
                  value={state?.current_highest_bid ? formatINR(state.current_highest_bid) : "—"}
                  accent="neon"
                />
                <Stat
                  label="Leading"
                  value={leadingTeam?.name || "—"}
                  accent={isLeading ? "neon" : "hot"}
                  highlight={isLeading}
                />
              </div>
              {isLeading ? (
              ...
              ) : tournament.max_players_per_team > 0 &&
                squad.length >= tournament.max_players_per_team ? (
                <div className="text-center py-6 rounded-xl bg-muted/20 border border-muted/50">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    You reached the {tournament.max_players_per_team} player limit.
                  </div>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  {minNext > team.remaining_purse && (
                    <p className="text-center text-xs text-destructive">
                      Insufficient purse for next bid.
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="text-center py-20 text-muted-foreground">
              ...
              <Button asChild variant="ghost" size="sm" className="mt-4">
                <Link to="/watch/$slug" params={{ slug: tournament.id }}>
                  Open spectator view
                </Link>
              </Button>
            </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              {squad.length > 0 && (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={downloadSquadSummary}
                  className="h-6 px-2 text-neon hover:text-neon hover:bg-neon/10"
                  title="Download Squad (CSV)"
                >
              ...
              <ul className="space-y-1.5 text-sm">
                {squad.map((p) => (
                  <li key={p.id} className="flex justify-between">
                    <span>{p.name}</span>
                    <span className="text-neon">{formatINR(p.sold_price)}</span>
                  </li>
                ))}
              </ul>
            )}
              ...
              {bids.map((b) => {
                ...
                return (
                  <li key={b.id} className="flex justify-between text-muted-foreground">
                    <span>{t?.name}</span>
                    <span className="text-foreground">{formatINR(b.amount)}</span>
                  </li>
                );
              })}
            </ul>
...
function Stat({
  ...
  return (
    <div
      className={`bg-card/60 rounded-lg p-4 border ${highlight ? "border-neon ring-neon" : "border-border"} min-w-0`}
    >
      <div className="text-xs text-muted-foreground truncate">{label}</div>
      <div className={`text-lg md:text-2xl font-bold text-${accent} truncate`} title={value}>
        {value}
      </div>
    </div>
  );
