...
function TeamRoom() {
  ...
  const sportSlug = SPORTS.find(s => tournament.name.toLowerCase().includes(s.slug))?.slug || "cricket";
  const sport = getSport(sportSlug);

  // call ticker hook
  return <TeamRoomInner team={team} tournament={tournament} state={state} players={players} allTeams={allTeams} bids={bids} currentPlayer={currentPlayer} leadingTeam={leadingTeam} minNext={minNext} isLeading={!!isLeading} timeLeft={timeLeft} isLive={!!isLive} canBid={!!canBid} squad={squad} bidding={bidding} placeBid={placeBid} soldPlayer={soldPlayer} soldTeam={soldTeam} soldPrice={state?.last_sold_price ?? null} sport={sport} />;
}

interface InnerProps { team: Team; tournament: Tournament; state: AuctionState|null; players: Player[]; allTeams: Team[]; bids: Bid[]; currentPlayer: Player|null; leadingTeam: Team|undefined; minNext: number; isLeading: boolean; timeLeft: number; isLive: boolean; canBid: boolean; squad: Player[]; bidding: boolean; placeBid: (n: number) => void; soldPlayer: Player|null|undefined; soldTeam: Team|null|undefined; soldPrice: number|null; sport: ReturnType<typeof getSport>; }

function TeamRoomInner({ team, tournament, state, allTeams, bids, currentPlayer, leadingTeam, minNext, isLeading, timeLeft, isLive, canBid, squad, bidding, placeBid, soldPlayer, soldTeam, soldPrice, sport }: InnerProps) {
