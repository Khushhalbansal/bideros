...
function Spectator() {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "auction_state",
          filter: `tournament_id=eq.${slug}`,
        },
        (payload) => {
          const ns = payload.new as AuctionState;
          setState(ns);
          if (ns?.current_highest_bid) {
            setFlash(true);
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "teams", filter: `tournament_id=eq.${slug}` },
        debouncedLoad,
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "players", filter: `tournament_id=eq.${slug}` },
        debouncedLoad,
      )
      .subscribe();
    return () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <HammerStrikes count={state?.strike_count ?? 0} />
      <AnimatePresence>
      ...
      {t.banner_url && (
        <img src={t.banner_url} alt="" className="w-full h-28 md:h-40 object-cover" />
      )}
      <header className="px-6 py-4 flex items-center justify-between border-b border-border bg-glass">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          <Link
            to="/"
            className="px-3 py-1.5 rounded-md border border-border hover:text-neon hover:border-neon transition"
          >
            ← Home
          </Link>
          <Link
      ...
      <main className="flex-1 flex flex-col lg:grid lg:grid-cols-[1fr_360px] gap-4 lg:gap-6 p-4 lg:p-6">
        <section className="flex flex-col gap-4 lg:gap-6 flex-1 min-h-0">
          <div
            className={`flex-1 rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl p-6 lg:p-10 flex flex-col justify-center items-center text-center relative overflow-hidden ${flash ? "ring-2 ring-neon shadow-[0_0_30px_rgba(var(--neon-rgb),0.3)]" : ""} transition-all duration-300`}
          >
            {isLive && currentPlayer ? (
              <div className="animate-slide-up w-full">
                <div className="text-[10px] lg:text-xs uppercase tracking-[0.3em] text-neon mb-2 lg:mb-4">
                  Now on the block
                </div>
                {currentPlayer.photo_url && (
                  <img
                    ...
                    className="mx-auto mb-3 lg:mb-4 h-24 w-24 md:h-40 md:w-40 rounded-full object-cover border-2 lg:border-4 border-neon/60 shadow-[0_0_30px_rgba(var(--neon-rgb),0.5)]"
                  />
                )}
                <div className="text-4xl md:text-6xl lg:text-8xl font-display font-bold mb-2 lg:mb-4 tracking-tight leading-none">
                  {currentPlayer.name}
                </div>
                ...
                {timeLeft != null && (
                  <div
                    className={`text-2xl lg:text-3xl font-bold mb-4 lg:mb-6 ${timeLeft <= 5 ? "text-hot animate-pulse-neon" : "text-neon"}`}
                  >
                    ⏱ {timeLeft}s
                  </div>
                )}
                <div className="grid grid-cols-2 gap-3 md:gap-6 max-w-3xl mx-auto w-full">
                  <div className="rounded-xl gradient-neon p-3 lg:p-6 text-primary-foreground shadow-[0_10px_30px_-10px_rgba(var(--neon-rgb),0.5)] min-w-0 border border-white/20">
                    <div className="text-[9px] lg:text-xs uppercase tracking-widest opacity-90 font-bold truncate">
                      Current bid
                    </div>
                    <div className="text-xl md:text-3xl lg:text-5xl font-black mt-1 truncate drop-shadow-md">
                  ...
                  <div className="rounded-xl gradient-hot p-3 lg:p-6 shadow-[0_10px_30px_-10px_rgba(255,51,102,0.5)] min-w-0 border border-white/20 text-white">
                    ...
                    <div
                      className="text-xl md:text-3xl lg:text-5xl font-black mt-1 truncate drop-shadow-md"
                      title={leading?.name || "—"}
                    >
                      {leading?.name || "—"}
                    </div>
              ...
              <div className="text-muted-foreground">
                ...
                <div className="text-lg lg:text-2xl font-display font-bold">
          ...
          <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl px-4 py-3 overflow-hidden">
            <div className="text-[10px] lg:text-xs uppercase tracking-widest text-neon mb-2 font-bold">
              Recent sales
            </div>
            <div className="flex gap-3 overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden">
              ...
              {soldHistory.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="min-w-[140px] lg:min-w-[180px] bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg px-3 py-2 transition-colors"
                  >
                    ...
                    <div className="text-xs lg:text-sm text-neon font-black mt-1">
                      {formatINR(p.sold_price)}
                    </div>
                  </div>
                );
              })}
        ...
        <aside className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl p-4 lg:p-6">
          <h3 className="font-bold mb-3 text-[10px] lg:text-xs uppercase tracking-widest text-neon">
            Teams Standings
          </h3>
            ...
            {[...teams]
              ...
              .map((tm) => {
                ...
                return (
                  <div
                    key={tm.id}
                    className={`shrink-0 min-w-[180px] lg:min-w-0 rounded-xl p-3 lg:p-4 border ${isLead ? "border-neon ring-2 ring-neon/20 bg-primary/20 shadow-[0_0_15px_rgba(var(--neon-rgb),0.3)]" : "border-white/10 bg-white/5 hover:bg-white/10"} transition-all`}
                  >
                    <div className="flex justify-between items-center mb-1 gap-2">
                      <div className="font-bold text-sm lg:text-base truncate">{tm.name}</div>
                      <div className="text-[10px] lg:text-xs text-white/50 bg-black/40 px-2 py-0.5 rounded-full whitespace-nowrap">
                        {squadCount} players
                      </div>
                    </div>
                    <div className="text-sm lg:text-lg text-neon font-black drop-shadow-sm mt-1">
