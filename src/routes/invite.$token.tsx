...
function InvitePage() {
  ...
  if (loading) content = <div className="text-muted-foreground">Loading…</div>;
    ...
    content = (
      <div className="text-muted-foreground p-8 text-center bg-glass border border-border rounded-xl">
        Invite not found.
      </div>
    );
  else if (info.used) {
    ...
    content = (
      <div className="text-muted-foreground p-8 text-center bg-glass border border-border rounded-xl">
        ...
        <Link to="/auth" className="text-neon ml-2">
          Sign in
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 text-xl font-bold font-display">{content}</div>
    </div>
  );
}
