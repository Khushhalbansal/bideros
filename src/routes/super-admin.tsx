...
function SuperAdminPanel() {
  ...
  const toggleBlock = async (id: string, blocked: boolean) => {
    const { data, error } = await supabase.rpc("sa_set_blocked", {
      p_tournament: id,
      p_blocked: !blocked,
    });
  ...
  const updateUserQuota = async (userId: string, change: number) => {
    const { data, error } = await supabase.rpc("sa_update_user_quota", {
      p_user_id: userId,
      p_change: change,
    });
    if (error) return toast.error(error.message);
  ...
  const updateFeedbackStatus = async (id: string, status: string) => {
    const { error } = await supabase.rpc("sa_update_feedback_status", {
      p_feedback_id: id,
      p_status: status,
    });
    if (error) return toast.error(error.message);
        ...
        <Tabs defaultValue="tournaments">
          <TabsList className="flex-wrap h-auto">
            ...
            <TabsTrigger value="feedback">
              Feedback ({feedback.filter((f) => f.status === "open").length})
            </TabsTrigger>
          ...
          <TabsContent value="tournaments" className="mt-6 space-y-2">
            {tournaments.map((t) => (
              <div
                key={t.id}
                className={`bg-glass border rounded-xl p-4 flex flex-wrap items-center justify-between gap-3 ${t.blocked ? "border-destructive/50" : "border-border"}`}
              >
                <div className="min-w-0">
                  <div className="font-bold flex items-center gap-2 flex-wrap">
                    ...
                    {t.blocked && (
                      <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-destructive/20 text-hot">
                        blocked
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {t.admin_email || t.admin_id} • {t.team_count} teams • {t.player_count} players
                  </div>
                </div>
                <div className="flex gap-2 flex-wrap">
                  ...
                  {t.status === "live" && (
                    <Button size="sm" variant="outline" onClick={() => forceEnd(t.id)}>
          ...
          <TabsContent value="users" className="mt-6 space-y-2">
            {users.map((u) => (
              <div
                key={u.id}
                className="bg-glass border border-border rounded-xl p-3 flex items-center justify-between gap-3"
              >
                <div>
                  ...
                  <div className="flex gap-1 flex-wrap mt-1">
                    ...
                    {u.roles.map((r) => (
                      <span
                        key={r}
                        className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-primary/15 text-neon"
                      >
                        {r}
                      </span>
                    ))}
                ...
                <div className="flex items-center gap-3 bg-background/50 rounded-lg p-2 border border-border/50">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="h-6 w-6"
                    onClick={() => updateUserQuota(u.id, -1)}
                  >
                    -
                  </Button>
          ...
          <TabsContent value="supers" className="mt-6 space-y-4">
            <form
              onSubmit={addSuper}
              className="bg-glass border border-border rounded-xl p-4 flex gap-2 items-end"
            >
              ...
              <Button className="gradient-neon text-primary-foreground shadow-neon">
            ...
            <div className="space-y-2">
              {supers.map((s) => (
                <div
                  key={s.email}
                  className="bg-glass border border-border rounded-xl p-3 flex items-center justify-between"
                >
                  <div>
                    <div className="font-semibold text-sm">{s.email}</div>
                    <div className="text-xs text-muted-foreground">
                      {s.user_id ? "active" : "pending first login"}
                    </div>
                  </div>
          ...
          <TabsContent value="audit" className="mt-6 space-y-1">
            {audit.map((a) => (
              <div
                key={a.id}
                className="bg-glass border border-border rounded-md px-3 py-2 text-xs flex items-center justify-between gap-3"
              >
                <div>
          ...
          <TabsContent value="feedback" className="mt-6 space-y-4">
            ...
            {[
              ...
              "upgrade",
              "other",
            ].filter((type) => type === "all" || feedback.some((f) => f.feedback_type === type))
              .length > 0 && (
              <div className="flex gap-2 flex-wrap text-xs">
                ...
                {[
                  ...
                  .filter(
                    (type) => type === "all" || feedback.some((f) => f.feedback_type === type),
                  )
                  .map((type) => (
                    <span
                      key={type}
                      className="px-3 py-1 rounded-full bg-muted border border-border text-muted-foreground font-bold capitalize"
                    >
                      {type}{" "}
                      {type !== "all"
            ...
            {feedback.map((f) => (
              <div
                key={f.id}
                className={`bg-glass border rounded-xl p-4 flex flex-col gap-3 ${f.status === "open" ? "border-neon/50 shadow-[0_0_15px_rgba(var(--neon),0.1)]" : "border-border opacity-70"}`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                            ...
                            : f.feedback_type === "feature"
                              ...
                              : f.feedback_type === "bug"
                                ...
                                : f.feedback_type === "design"
                                  ...
                                  : f.feedback_type === "performance"
                                    ? "bg-yellow-500/20 text-yellow-400"
                                    : f.feedback_type === "issue"
                                      ? "bg-hot/20 text-hot"
                                      : f.feedback_type === "review"
                                        ? "bg-yellow-500/20 text-yellow-500"
                                        : f.feedback_type === "upgrade"
                      ...
                      {f.rating && (
                        <span className="text-xs font-bold text-yellow-500">★ {f.rating}/5</span>
                      )}
                      <span
                    ...
                    {f.page_url && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs text-muted-foreground hover:underline hover:text-neon truncate max-w-[200px] block"
                      >
                        {f.page_url}
                      </a>
                    )}
                ...
                {f.content && (
                  <div className="text-sm bg-background/50 p-3 rounded-lg border border-border/50 whitespace-pre-wrap">
                    {f.content}
                  </div>
                )}
                {f.screenshot_url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block max-w-[300px] mt-2 rounded-lg overflow-hidden border border-border hover:border-neon transition-colors"
                  >
                    <img
...
function PricingSettingsEditor() {
  ...
  const saveConfig = async () => {
    const { error } = await supabase.rpc("sa_update_setting", {
      p_key: "pricing_config",
      p_value: config,
    });
    if (error) return toast.error(error.message);
  ...
  return (
    <div className="bg-glass border border-border rounded-xl p-6 space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        Changes here immediately reflect on the /pricing page for all users.
      </p>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              value={config.headline_highlight}
              onChange={(e) => setConfig({ ...config, headline_highlight: e.target.value })}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <Label>Price (₹)</Label>
              <Input
                value={config.single_price}
                onChange={(e) => setConfig({ ...config, single_price: e.target.value })}
              />
            </div>
            <div className="flex-1">
              <Label>Strike Price (₹)</Label>
              <Input
                value={config.single_price_strike}
                onChange={(e) => setConfig({ ...config, single_price_strike: e.target.value })}
              />
            </div>
          </div>
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setConfig({ ...config, single_features: e.target.value.split("\n") })
              }
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <div className="flex-1">
              <Label>Strike Price (₹)</Label>
              <Input
                value={config.monthly_price_strike}
                onChange={(e) => setConfig({ ...config, monthly_price_strike: e.target.value })}
              />
            </div>
          </div>
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setConfig({ ...config, monthly_features: e.target.value.split("\n") })
              }
            />
          </div>
        </div>
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              ...
              <Input
                ...
                onChange={(e) => setConfig({ ...config, yearly_price: e.target.value })}
              />
            </div>
            <div className="flex-1">
              <Label>Strike Price (₹)</Label>
              <Input
                value={config.yearly_price_strike}
                onChange={(e) => setConfig({ ...config, yearly_price_strike: e.target.value })}
              />
            </div>
          </div>
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setConfig({ ...config, yearly_features: e.target.value.split("\n") })
              }
            />
          </div>
        </div>
      ...
      <div className="flex justify-end mt-8 border-t border-border/50 pt-4">
        <Button onClick={saveConfig} className="gradient-neon text-primary-foreground">
          Save Configuration
        </Button>
      </div>
    </div>
