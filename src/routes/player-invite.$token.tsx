...
function PlayerInvitePage() {
  ...
  const sportSlug = SPORTS.find(s => info.tournament_name.toLowerCase().includes(s.slug))?.slug || "cricket";
  const sport = getSport(sportSlug);

  return (
    <div className="min-h-screen flex flex-col text-white relative overflow-hidden" style={{ backgroundColor: sport.bg }}>
      ...
      <header className="container mx-auto py-6 px-4 relative z-10"><Logo /></header>
      <main className="flex-1 flex items-center justify-center px-4 pb-12 relative z-10">
        <div className="w-full max-w-md bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl animate-slide-up" style={{ boxShadow: `0 0 80px -20px ${sport.accent}66` }}>
          <div className="text-xs uppercase tracking-widest text-neon mb-1" style={{ color: sport.accent }}>Tournament invite</div>
