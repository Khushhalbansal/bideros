    ...
    try {
      ...
      const inserts = dataRows.map((cells, i) => {
        ...
        const photo_url = photoRef
          ? photoMap.get(photoRef) || photoMap.get(photoRef.replace(/\.[^.]+$/, "")) || null
          : null;
      ...
      toast.success(
        `Imported ${inserts.length} players${photoMap.size ? ` • ${photoMap.size} photos matched` : ""}`,
      );
      setText("");
      setPhotoFiles([]);
      setOpen(false);
  ...
  const fieldSelect = (
    ...
    <div>
      <Label className="text-xs">
        {label}
        {!optional && <span className="text-hot">*</span>}
      </Label>
    ...
    <>
      <div className="bg-glass border border-border rounded-xl p-5 space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Upload your own CSV with any columns. Map them in the next step. Add a folder of photos to
          match by filename.
        </p>
          ...
          <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
            <div className="grid grid-cols-3 gap-3 items-end">
              ...
              <div className="text-xs text-muted-foreground text-right">
                {dataRows.length} data row{dataRows.length === 1 ? "" : "s"} · {columnCount} column
                {columnCount === 1 ? "" : "s"}
              </div>
            </div>
            ...
            {map.photo >= 0 && (
              <div className="bg-glass border border-neon/30 rounded-lg p-3 space-y-2">
                <Label className="text-xs">
                  Photo folder — pick all images, we'll match by filename in the photo column
                </Label>
                <Input
                  type="file"
                ...
                {photoFiles.length > 0 && (
                  <p className="text-[10px] text-muted-foreground">
                    {photoFiles.length} photos selected · max 5MB each
                  </p>
                )}
              </div>
            )}
