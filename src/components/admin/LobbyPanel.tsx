    ...
    <div className="grid md:grid-cols-2 gap-6">
      <div className="bg-glass border border-border rounded-xl p-5">
        <h3 className="font-bold flex items-center gap-2 mb-3">
          ...
          Team owners ({teams.filter((t) => t.owner_linked).length}/{teams.length})
        </h3>
        ...
        <div className="space-y-2 max-h-[420px] overflow-auto">
          {teams.map((t) => (
            <div
              key={t.id}
              className="flex items-center gap-3 bg-card/40 rounded-md px-3 py-2 border border-border"
            >
              {t.logo_url ? (
                <img
                  src={t.logo_url}
              ...
              {t.owner_linked ? (
                <span className="text-[10px] text-neon font-bold">✓ JOINED</span>
              ) : (
      ...
      <div className="bg-glass border border-border rounded-xl p-5">
        <h3 className="font-bold flex items-center gap-2 mb-3">
          ...
          {selfPlayers.length > 0 && (
            <span className="text-[10px] text-neon ml-2">{selfPlayers.length} self-registered</span>
          )}
        </h3>
