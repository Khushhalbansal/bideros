...
export function CategoriesTab({
  ...
  const remove = async (id: string) => {
    if (
      !confirm(
        "Delete category? Players will become uncategorised (default tournament base/increment).",
      )
    )
      return;
    ...
    <div className="grid md:grid-cols-3 gap-6">
      <form onSubmit={add} className="bg-glass border border-border rounded-xl p-5 space-y-3 h-fit">
        ...
        <p className="text-xs text-muted-foreground">
          Categories are optional. Players in a category share the same base price and bid
          increment.
        </p>
        <div>
          <Label>Name</Label>
          <Input
            ...
            placeholder="Marquee / A / B / Uncapped"
            required
          />
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <Input value={base} onChange={(e) => setBase(e.target.value)} placeholder="2 Cr" />
          </div>
          <div>
            <Label>Increment</Label>
            <Input value={incr} onChange={(e) => setIncr(e.target.value)} placeholder="20 L" />
          </div>
        </div>
        <Button
          disabled={busy}
      ...
      <div className="md:col-span-2 space-y-2">
        {items.length === 0 && (
          <div className="bg-glass border border-dashed border-border rounded-xl p-8 text-center text-sm text-muted-foreground">
            No categories yet — all players will be treated as a single "General" pool with the
            tournament default base price and bid increment.
          </div>
        )}
        {items.map((c) => (
          <div
            key={c.id}
            className="bg-glass border border-border rounded-xl p-3 grid grid-cols-1 sm:grid-cols-[1fr_auto_auto_auto] gap-2 items-center"
          >
            <Input
              defaultValue={c.name}
              onBlur={(e) => {
                if (e.target.value !== c.name) update(c.id, { name: e.target.value });
              }}
              className="h-8 text-sm font-semibold"
            />
            <Input
              defaultValue={String(c.base_price)}
              onBlur={(e) => {
                const v = parseINR(e.target.value);
                if (v && v !== c.base_price) update(c.id, { base_price: v });
              }}
              className="h-8 text-xs font-mono w-28"
            />
            <Input
              defaultValue={String(c.min_bid_increment)}
              onBlur={(e) => {
                const v = parseINR(e.target.value);
                if (v && v !== c.min_bid_increment) update(c.id, { min_bid_increment: v });
              }}
              className="h-8 text-xs font-mono w-28"
            />
        ...
        {items.length > 0 && (
          <p className="text-[10px] text-muted-foreground pt-2 flex items-center gap-1">
            <Save className="h-3 w-3" />
            Click any field to edit. Changes save on blur.
          </p>
        )}
      </div>
    </div>
