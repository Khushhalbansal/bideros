  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from("profiles")
        .select("points, auctions_quota, referral_code")
        .eq("id", userId)
        .single();
      setProfile(data);
    })();
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
        <div className="flex-1 space-y-4">
          ...
          <p className="text-muted-foreground text-sm max-w-xl">
            Invite your friends to host tournaments on Bideros. When they buy a Single Tournament
            pass, you earn <strong className="text-neon">20 points</strong>. Reach 120 points for a
            free auction! If they buy a Pro Subscription, you instantly get a free auction!
          </p>
        ...
        <div className="flex-1 w-full bg-background/50 border border-border rounded-xl p-5 space-y-4">
          <div className="flex justify-between items-end">
            <div>
              ...
              <div className="text-3xl font-black text-neon">
                {points} <span className="text-sm text-muted-foreground font-medium">/ 120</span>
              </div>
            </div>
          ...
          <div className="flex justify-between items-center pt-2">
            <span className="text-xs text-muted-foreground">
              {120 - (points % 120)} points to next free auction
            </span>
            <Button
              asChild
              size="sm"
              variant="outline"
              className="h-7 text-xs border-neon/50 text-neon hover:bg-neon/10"
            >
              <Link to="/pricing">
